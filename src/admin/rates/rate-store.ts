...
export class RateStore {
	...
	createSellRate(input: SellRateInput): SellRate {
		...
		this.db
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			...
			.run(id, input.capability, input.displayName, input.unit, input.priceUsd, input.model ?? null, isActive ? 1 : 0, sortOrder, now, now);
	...
	listSellRates(filters?: RateFilters): { entries: SellRate[]; total: number } {
		...
		const total = (
			this.db.prepare(`SELECT COUNT(*) as count FROM sell_rates ${whereClause}`).get(...whereValues) as { count: number }
		).count;
	...
	createProviderCost(input: ProviderCostInput): ProviderCost {
		...
		if (!input.model) {
			...
			if (existing) {
				throw new Error(`A provider cost with capability '${input.capability}', adapter '${input.adapter}', and NULL model already exists`);
			}
		}
		...
		this.db
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			...
			.run(id, input.capability, input.adapter, input.model ?? null, input.unit, input.costUsd, priority, latencyClass, isActive ? 1 : 0, now, now);
	...
	updateProviderCost(id: string, input: Partial<ProviderCostInput>): ProviderCost {
		...
		if ("model" in input && !input.model) {
			...
			if (duplicate) {
				throw new Error(`A provider cost with capability '${existing.capability}', adapter '${existing.adapter}', and NULL model already exists`);
			}
		}
	...
	listProviderCosts(filters?: ProviderCostFilters): { entries: ProviderCost[]; total: number } {
		...
		const total = (
			this.db.prepare(`SELECT COUNT(*) as count FROM provider_costs ${whereClause}`).get(...whereValues) as { count: number }
		).count;

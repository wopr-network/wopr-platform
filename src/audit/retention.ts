...
export async function purgeExpiredEntries(repo: IAuditLogRepository): Promise<number> {
...
export async function purgeExpiredEntriesForUser(repo: IAuditLogRepository, userId: string): Promise<number> {
  const retentionMs = RETENTION_DAYS * 24 * 60 * 60 * 1000;
  const cutoff = Date.now() - retentionMs;

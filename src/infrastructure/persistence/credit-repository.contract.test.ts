...
function runRepositoryContractTests(name: string, createRepo: () => Promise<CreditRepository> | CreditRepository) {
  describe(name, () => {
    ...
    describe("credit", () => {
      it("should add credits to a new tenant", async () => {
        const transaction = await repo.credit(tenantId, Money.fromCents(1000), "purchase", "Test purchase", "ref-123");
    ...
    describe("debit", () => {
      ...
      it("should deduct credits", async () => {
        const transaction = await repo.debit(tenantId, Money.fromCents(300), "adapter_usage");
      ...
      it("should reject insufficient balance", async () => {
        await expect(repo.debit(tenantId, Money.fromCents(1500), "adapter_usage")).rejects.toThrow(

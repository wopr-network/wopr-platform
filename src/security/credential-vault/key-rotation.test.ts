...
describe("reEncryptAllCredentials", () => {
  ...
  it("re-encrypts tenant BYOK keys from old to new secret", () => {
    ...
    const row = db.prepare("SELECT encrypted_key FROM tenant_api_keys WHERE id = ?").get("tk-1") as {
  ...
  it("handles mixed valid and invalid rows gracefully", () => {
    ...
    db.prepare("INSERT INTO provider_credentials (id, encrypted_value) VALUES (?, ?)").run("cred-2", "not-valid-json");
  ...
  it("re-encrypts multiple provider credentials", () => {
    ...
    const newKey = getVaultEncryptionKey(NEW_SECRET);
    for (let i = 1; i <= 3; i++) {
      const row = db.prepare("SELECT encrypted_value FROM provider_credentials WHERE id = ?").get(`cred-${i}`) as {
        encrypted_value: string;
      };
      const payload: EncryptedPayload = JSON.parse(row.encrypted_value);

...
async function insertKey(pool: PGlite, tenantId: string, provider: string, encryptedKey: string): Promise<void> {
  const now = Date.now();
  await pool.query(
    "INSERT INTO tenant_api_keys (id, tenant_id, provider, label, encrypted_key, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)",
    [randomUUID(), tenantId, provider, "", encryptedKey, now, now],
  );
}
...
describe("DrizzleKeyResolutionRepository", () => {
  ...
  it("returns the encrypted key for a matching tenant and provider", async () => {
    ...
    await insertKey(pool, "tenant-1", "anthropic", payload);
  ...
  it("isolates keys between tenants (cross-tenant isolation)", async () => {
    ...
    await insertKey(pool, "tenant-a", "anthropic", payloadA);
    await insertKey(pool, "tenant-b", "anthropic", payloadB);
  ...
  it("tenant A cannot see tenant B keys", async () => {
    ...
    await insertKey(pool, "tenant-b", "openai", payload);
  ...
  it("returns correct key when tenant has multiple providers", async () => {
    ...
    await insertKey(pool, "tenant-1", "anthropic", anthropicPayload);
    await insertKey(pool, "tenant-1", "openai", openaiPayload);

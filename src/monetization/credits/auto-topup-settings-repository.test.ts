...
describe("DrizzleAutoTopupSettingsRepository", () => {
  ...
  it("upsert creates settings and getByTenant retrieves them", async () => {
    await repo.upsert("t1", { usageEnabled: true, usageThresholdCredits: 200, usageTopupCredits: 1000 });
    const s = await repo.getByTenant("t1");
    expect(s).not.toBeNull();
    expect(s?.usageEnabled).toBe(true);
    expect(s?.usageThresholdCredits).toBe(200);
    expect(s?.usageTopupCredits).toBe(1000);
    expect(s?.scheduleEnabled).toBe(false);
  });

  it("upsert updates existing settings", async () => {
    await repo.upsert("t1", { usageEnabled: true });
    await repo.upsert("t1", { usageThresholdCredits: 300 });
    const s = await repo.getByTenant("t1");
    expect(s?.usageEnabled).toBe(true);
    expect(s?.usageThresholdCredits).toBe(300);
  });
  ...
  it("listDueScheduled returns tenants with schedule_next_at <= now", async () => {
    ...
    await repo.upsert("t1", { scheduleEnabled: true, scheduleAmountCredits: 500, scheduleNextAt: past });
    await repo.upsert("t2", { scheduleEnabled: true, scheduleAmountCredits: 1000, scheduleNextAt: future });
